...
design system. It is the org's one generated repository: the bundle is
emitted by theme's `cmd/vg-tokens` and pushed to `claude.ai/design` by the
org's `scripts/push-design.sh`, so a change to a token lands in theme first
and is re-emitted here rather than edited here. Its root module imports
nothing else in the organization. Nothing in the organization imports it.
